...
const STEPS = [
  ...
  ['05', 'Weekly check-ins', 'We check in every week to keep the plan moving, answer what comes up, and adjust before small problems become setbacks.'],
  ['06', 'Retest and reevaluate', 'We re-run your bloodwork to see whether we made the changes we set out to make, then reevaluate your symptoms and your goals for the next block of work.'],
];
...
function HomeScreen({ go }) {
  ...
  return (
    <>
      <section className="lux-section" style={{ background: 'var(--navy-900)', padding: '120px 5% 100px', position: 'relative' }}>
      ...
      <Section tone="dark">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 'var(--space-5)', marginTop: 56 }}>
          {STEPS.map(([n, t, d]) => (
            <div key={n} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', borderTop: '1px solid var(--border-inverse)', paddingTop: 'var(--space-5)' }}>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 48, lineHeight: 1, letterSpacing: '-0.02em', color: 'rgba(201,168,76,.35)' }}>{n}</span>
