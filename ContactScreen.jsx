  ...
  return (
    <div style={{ position: 'relative' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.15fr .85fr', gap: '5rem', alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-8)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
              ...
              <Input label="Email address" placeholder="you@example.com" hint="We reply within one business day." />
              <Select label="I am a…" options={['Patient / individual', 'Chiropractor or clinic owner', 'Personal trainer', 'Other healthcare provider']} />
            ...
            <Textarea label="Message" rows={4} placeholder="Tell us what you're looking for or what questions you have…" />
            ...
            <div style={{ display: 'flex', gap: 'var(--space-4)', alignItems: 'center', flexWrap: 'wrap' }}>
              <Button size="lg" disabled={!consent} onClick={() => setOpen(true)}>Request {slot}</Button>
      ...
      <Dialog open={open} eyebrow="Confirm" title="Request this consultation?" onClose={() => setOpen(false)}
        footer={<><Button variant="ghost" onClick={() => setOpen(false)}>Back</Button><Button onClick={() => { setOpen(false); setToast(true); }}>Confirm request</Button></>}>
      ...
      {toast && (
        <div style={{ position: 'fixed', right: 24, bottom: 24, zIndex: 50 }}>
          <Toast tone="success" title="Request sent" message={`We'll confirm ${slot} by email within one business day.`} onDismiss={() => setToast(false)} />
        </div>
      )}
    </div>
  );
